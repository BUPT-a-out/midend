...
void LoggerConfig::setModuleLogLevel(const std::string& moduleName, LogLevel level) {
    moduleLogLevels_[moduleName] = level;
}

LogLevel LoggerConfig::getEffectiveLogLevel(const std::string& moduleName) const {
...
std::ostream& LoggerConfig::getOutputStream() const {
    return *outputStream_;
}

std::mutex& LoggerConfig::getOutputMutex() const {
    return outputMutex_;
}

void LoggerConfig::enableTimestamps(bool enable) {
    enableTimestamps_ = enable;
}

void LoggerConfig::enableThreadIds(bool enable) {
    enableThreadIds_ = enable;
}

void LoggerConfig::enableColors(bool enable) {
...
bool LoggerConfig::threadIdsEnabled() const {
    return enableThreadIds_.load();
}

bool LoggerConfig::colorsEnabled() const {
    return enableColors_.load();
}
...
void LoggerConfig::loadFromEnvironment() {
    ...
    if (const char* level = std::getenv("MIDEND_LOG_LEVEL")) {
        ...
        if (levelStr == "DEBUG") setGlobalLogLevel(LogLevel::DEBUG);
        else if (levelStr == "INFO") setGlobalLogLevel(LogLevel::INFO);
        else if (levelStr == "WARNING") setGlobalLogLevel(LogLevel::WARNING);
        else if (levelStr == "ERROR") setGlobalLogLevel(LogLevel::ERROR);
        else if (levelStr == "NONE") setGlobalLogLevel(LogLevel::NONE);
    }
    ...
    if (const char* modules = std::getenv("MIDEND_LOG_MODULES")) {
        ...
        while (pos < modulesStr.length()) {
            ...
            std::string levelStr = modulesStr.substr(colonPos + 1, commaPos - colonPos - 1);
            ...
            else if (levelStr == "NONE") level = LogLevel::NONE;
...
LogStream::LogStream(LogLevel level, const std::string& moduleName, bool shouldLog)
    : level_(level), moduleName_(moduleName), shouldLog_(shouldLog), 
      isFirstLine_(true), moved_(false) {
}
...
void LogStream::processMultilineContent(const std::string& content, std::ostream& out) const {
    ...
    while (std::getline(iss, line)) {
        if (!firstLine) {
            ...
            if (!moduleName_.empty() && moduleName_ != "General") {
                prefixPadding += "[" + std::string(moduleName_.length(), ' ') + "] ";
            }
