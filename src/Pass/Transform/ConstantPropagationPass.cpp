...
void ConstantPropagationPass::replaceWithConstant(Instruction* inst, Constant* c) {
...
Constant* ConstantPropagationPass::foldBinaryOp(unsigned opcode, Constant* lhs,
    ...
    if (lhsInt && rhsInt) {
        ...
        switch (static_cast<Opcode>(opcode)) {
            ...
            case Opcode::Div:
                if (rhsVal != 0) {
                    result = static_cast<int64_t>(lhsVal) / static_cast<int64_t>(rhsVal);
                } else {
                    return nullptr;
                }
                break;
...
Constant* ConstantPropagationPass::foldCmpInst(unsigned predicate, Constant* lhs,
                                               Constant* rhs) {
    auto* lhsInt = dyn_cast<ConstantInt>(lhs);
    auto* rhsInt = dyn_cast<ConstantInt>(rhs);
