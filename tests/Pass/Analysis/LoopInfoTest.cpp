...
class LoopInfoTest : public ::testing::Test {
   protected:
    ...
    std::pair<Function*, std::unique_ptr<LoopInfo>> analyzeFunction(
        ...
        auto loopInfo = std::make_unique<LoopInfo>(func, &domInfo);
...
exit:
  ret i32 %i
    ...
    EXPECT_TRUE(loop->contains(func->getBasicBlocks()[1]));   // loop_header
    EXPECT_TRUE(loop->contains(func->getBasicBlocks()[2]));   // loop_body
    EXPECT_FALSE(loop->contains(func->getBasicBlocks()[0]));  // entry
    EXPECT_FALSE(loop->contains(func->getBasicBlocks()[3]));  // exit
...
TEST_F(LoopInfoTest, NestedLoopsTwoLevels) {
    ...
    const std::string expectedIR = R"(
        define i32 @nested_loops(i32 %arg0, i32 %arg1) {
        entry:
...
TEST_F(LoopInfoTest, MultipleBackedgeLoop) {
    ...
    // Check that both body1 and body2 are latches
    std::set<std::string> latchNames;
    for (auto* latch : latches) {
        latchNames.insert(latch->getName());
    }
    EXPECT_TRUE(latchNames.count("body1"));
    EXPECT_TRUE(latchNames.count("body2"));
}
...
TEST_F(LoopInfoTest, MultiBlockLoopBody) {
    ...
    EXPECT_EQ(loop->getBlocks().size(), 5u);  // header + 3 body parts + latch

    std::set<std::string> expectedBlocks = {
        "loop_header", "body_part1", "body_part2", "body_part3", "latch"};

    std::set<std::string> actualBlocks;
    for (auto* bb : loop->getBlocks()) {
        actualBlocks.insert(bb->getName());
    }
...
TEST_F(LoopInfoTest, IfElseInLoop) {
    ...
    // Test that all conditional blocks are properly identified as loop blocks
    EXPECT_EQ(loop->getBlocks().size(), 4u);  // header + if + else + latch

    std::set<std::string> expectedBlocks = {"loop_header", "if_branch",
...
TEST_F(LoopInfoTest, NoLoopsInFunction) {
    ...
    const std::string expectedIR = R"(
        define i32 @no_loops(i32 %arg0, i32 %arg1) {
        entry:
...
TEST_F(LoopInfoTest, DISABLED_LoopPropertiesAndMethods) {
    auto createFunc = [&]() -> Function* {
        auto* int32Ty = context->getInt32Type();
        auto* fnTy = FunctionType::get(int32Ty, {int32Ty});
...
// Test 18: Loop Print and Debug Functions
TEST_F(LoopInfoTest, LoopPrintAndDebug) {
    auto createFunc = [&]() -> Function* {
        auto* int32Ty = context->getInt32Type();
        auto* fnTy = FunctionType::get(int32Ty, {int32Ty});
        auto* func = Function::Create(fnTy, "print_test", module.get());
    ...
    // Test print methods (if available)
    loopInfo->print();
    auto* loop = loopInfo->getTopLevelLoops()[0].get();
    loop->print();
